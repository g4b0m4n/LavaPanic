var Ngotas,frames=0,change_tile=false, context, controller, level = 1, loop, width, height;
...
class Gout {
    ...
    colision(object) {
        ...
        if (object.x > this.x && object.y > this.y
            && object.h < this.h && object.w < this.w) {
...
let gout;
var goutPlop = function() {
    
    setTimeout(() => {
    if (gout == null){
        gout = new Gout();
        console.log("# gotas: "+Ngotas);
        //velocity aka level
        ...
        //shape
        context.fillRect(gout.x, gout.y, gout.w, gout.h);
        gout.y += Number(level);
        
    } else if (gout.colision(steve)
    | gout.colision(bucket) | gout.y > 400) {
        gout = null;
        console.log("gout dissapear");
    }},
        1000-(level*100));
};
...
controller = {
    ...
    mouseListener: function(event) {
        ...
        if (event.clientY < 200) {controller.up = touch_state;}
    },
...
loop = function() {
    ...
    // if steve goes past right boundary
        ...
        setTimeout(() => {steve2.x = current;}, 5000);
        steve2.x_velocity *= -1;
    ...
    if (steve2.last_left) {
        context.drawImage(document.getElementById(bucket2.tile), bucket2.xl, bucket2.y, 50, 50);
    } else {
        context.drawImage(document.getElementById(bucket2.tile), bucket2.xr, bucket2.y, 50, 50);
    }
